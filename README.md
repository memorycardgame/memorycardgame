# 🧠 Memory Card Game

<div align="center">

![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![License](https://img.shields.io/badge/License-MIT-green?style=for-the-badge)

A beautiful memory card matching game with animations and local storage support.

[🎮 Play Now](https://memorycardgame.github.io) · [🐛 Report Bug](https://github.com/memorycardgame/memorycardgame.github.io/issues) · [✨ Request Feature](https://github.com/memorycardgame/memorycardgame.github.io/issues)

</div>

---

## ✨ Features

- **3 Difficulty Levels** - Easy, Medium, Hard
- **4 Card Themes** - Animals, Food, Nature, Space
- **Score Tracking** - Moves, time, and points
- **High Scores** - Top 10 leaderboard
- **Local Storage** - Auto-save game progress
- **Animations** - 3D flip, confetti, and more
- **Responsive** - Works on all devices
- **Hint System** - 3 hints per game

---

## 🚀 Getting Started

### Play Online
Visit **[https://memorycardgame.github.io](https://memorycardgame.github.io)**

### Run Locally

```bash
git clone https://github.com/memorycardgame/memorycardgame.github.io.git
cd memorycardgamej.github.io
open index.html
```

---

## 🎯 How to Play

1. Click on cards to flip them
2. Find and match all pairs
3. Complete the game with fewer moves for higher score
4. Use hints if stuck (costs points)

---

## 🎮 Controls

| Action | Control |
|--------|---------|
| Flip Card | Click/Tap |
| New Game | 🔄 Button |
| Pause | ⏸️ Button |
| Hint | 💡 Button |
| Theme | 🎨 Button |

---

## 🛠️ Technologies

- HTML5
- CSS3
- JavaScript (ES6+)
- Web Audio API
- Local Storage API

---

## 📁 Project Structure

```
memorycardgame.github.io/
├── index.html
├── README.md
└── LICENSE
```

---

## 🤝 Contributing

1. Fork the repository
2. Create feature branch (`git checkout -b feature/NewFeature`)
3. Commit changes (`git commit -m 'Add NewFeature'`)
4. Push to branch (`git push origin feature/NewFeature`)
5. Open a Pull Request

---

## 📄 License

MIT License

---

## 🔗 Links

- **Repository**: [github.com/memorycardgame/memorycardgame.github.io](https://github.com/memorycardgame/memorycardgame.github.io)
- **Live Demo**: [memorycardgame.github.io](https://memorycardgame.github.io)

---

<div align="center">

⭐ Star this repo if you like it!

</div>
